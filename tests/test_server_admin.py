...
async def test_provision_agent_is_idempotent_on_repeated_call(monkeypatch):
    """Calling provision_agent twice with the same login must not create a
    duplicate account -- the tool just delegates to AdminClient.provision_agent,
    whose own idempotency is covered in test_admin.py; here we check the MCP
    tool doesn't do anything extra (e.g. pre-create) that would break that."""
    _clear_env(monkeypatch)
    monkeypatch.setenv("YOUTRACK_ADMIN_TOKEN", "perm-admin")
    mod = _reload_server()
    fake = _FakeAdminClient()
    ...
    tool = mod.mcp._tool_manager.get_tool("provision_agent")
    first = await tool.fn(login="agent-a", name="Agent A")
    second = await tool.fn(login="agent-a", name="Agent A")

    assert first == second == {"login": "agent-a", "token": "perm-secret-token"}
    assert fake.calls == [
        ("provision_agent", "agent-a", "Agent A"),
        ("provision_agent", "agent-a", "Agent A"),
    ]
